...
import FBSDKCoreKit
class ShowNotificationViewController: UIViewController,CLLocationManagerDelegate,MKMapViewDelegate,UIScrollViewDelegate  {
    var event_id : String!
    ...
    func addEvent(){
        let urlPath: String = "http://localhost:3000/updatePlayer/"+self.event_id
                ...
                if self.showEvent.join.count == self.showEvent.max {
                    ...
                    alertController.addAction(UIAlertAction(title: "Ok", style: .Default, handler: { (action: UIAlertAction!) in
                        ...
                        for i in 0...self.showEvent.join.count-1 {
                            ...
                            let parameter = [
                                "noti" : ["user_id":user,"date":day+"T"+String(change)+hour+".000Z","name":self.userName,"title":"Reserved an event","image":self.firstName,"eventId":self.showEvent.event_id],"_csrf":self.key
                            ]
